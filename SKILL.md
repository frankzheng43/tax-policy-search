...
默认只搜**文件类**（法律 / 行政法规 / 国务院文件 / 税务部门规章 / 税务规范性文件 / 财税文件 / 其他文件 / 工作通知 / 政策指引 / 文字政策解读），已滤掉新闻、视频、各地动态、互动交流。要搜全站传 `"label": ""`。
...
| 只搜某个栏目 | `"column": "政策法规"`（**只能单个值**，逗号拼多个会静默失效） |
...
10. **默认已限「文件类」** — 不加 `label` 白名单时，「小微企业」这类宽泛词会返回 5000+ 条（全是新闻、视频、各地动态），加白名单后 189 条。脚本已默认加，传 `"label": ""` 可关掉
11. **`column` 不认多值** — `"column": "政策法规,政策解读"` 会**静默回落成「全部」**，只能传单个值。政策法规栏目共 4970 篇（全站 42916 = 含新闻视频）
